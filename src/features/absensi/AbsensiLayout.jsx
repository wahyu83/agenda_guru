...
import { UserCog, QrCode, FileText } from 'lucide-react';
  ...
  return (
    <div className="mobile-layout">
      <header className="mobile-header shadow-sm">
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <span style={{ fontSize: '0.875rem', fontWeight: '600' }}>{user?.nama || 'Petugas Absensi'}</span>
            <span style={{ fontSize: '0.65rem', opacity: 0.8 }}>Petugas Absensi</span>
          </div>
        </div>
      </header>
      ...
      <nav className="bottom-nav">
        <NavLink to="/absensi" className={isActive('/absensi') ? 'nav-item active' : 'nav-item'}>
          <QrCode size={22} />
          <span>Absensi QR</span>
        </NavLink>
        <NavLink to="/absensi/laporan" className={isActive('/absensi/laporan') ? 'nav-item active' : 'nav-item'}>
          <FileText size={22} />
          <span>Laporan</span>
        </NavLink>
      </nav>
